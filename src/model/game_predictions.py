...
def simulate_tournament (bracket, team_stats, features, model_mcmc=None, coefs=None):
    ...
    # Truncate team statistics for easier merging.
    team_stats_truncated = team_stats[team_stats.TeamName.isin(teams)].copy()
    ...
    for c in coefs:
        ...
        # Simulate.
        c_outcome = simulate_bracket_coefs(bracket, team_stats_truncated, features, c)
...
def simulate_bracket_coefs (bracket, team_stats, features, coefs):
    ...
    Inputs:
        ...
        team_stats: Dataframe with team statistics.
        features:   List of features in data dataframe.
        coefs:      Numpy array of one set of coefficients; each column corresponds to an element of 'features'.
    Returns:
    ...
    # Create DF.
    games_df = pd.DataFrame(shuffled_bracket, columns=['team','opponent'])

    # Append presumed accurate location columns. Assume all locations neutral.
    games_df['location_Neutral'] = 1
    for c in ['location_Home','location_SemiAway','location_SemiHome']:
        games_df[c] = 0

    # Trim team stats to only the teams represented.
    team_stats = team_stats[(team_stats.TeamName.isin(games_df.team)) | (team_stats.TeamName.isin(games_df.opponent))]

    # Merge with KenPom data.
    # Merge teams.
    games_df = games_df.merge(team_stats, left_on='team', right_on='TeamName')
    games_df.drop('TeamName', axis=1, inplace=True)
    games_df.columns = ['team_'+c if (c in team_stats.columns and c != 'year') else c for c in games_df.columns]
    # Merge opponents.
    games_df = games_df.merge(team_stats, left_on='opponent', right_on='TeamName')
    games_df.drop('TeamName', axis=1, inplace=True)
    games_df.columns = ['opponent_'+c if (c in team_stats.columns and c != 'year') else c for c in games_df.columns]
    ...
    diff_vals  = np.array(games_df.ix[:,team_cols]) - np.array(games_df.ix[:,opponent_cols])
    ratio_vals = np.array(games_df.ix[:,team_cols]) / np.array(games_df.ix[:,opponent_cols])
    # Convert to DF.
    ...
    games_df = pd.concat((games_df,diff_vals_df),  axis=1)
    games_df = pd.concat((games_df,ratio_vals_df), axis=1)
    ...
    # Get winners/losers.
    winners = bracket.ravel()[winners_losers == 1]
    losers  = bracket.ravel()[winners_losers == 0]
